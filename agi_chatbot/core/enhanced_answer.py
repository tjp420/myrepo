...
# Optional NumPy import (used for simple feature vectors). Use optional_import
# so tests/CI without NumPy won't fail at import-time; code should guard usage
# if NumPy isn't available at runtime.
np = optional_import("numpy", fallback=None, warn_msg="NumPy not available")
...
try:
    from .capability_disclosure import (
        is_ai_limitations_query,
        is_capability_query,
        is_coding_improvement_query,
        is_enhancement_query,
        is_speech_improvement_query,
        produce_ai_limitations_response,
        produce_capability_response,
        produce_coding_improvement_response,
        produce_enhancement_response,
        produce_speech_improvement_response,
    )
...
try:
    from .advanced_reasoning import enhance_response_with_advanced_reasoning
...
try:
    from agi_chatbot_accretor_integration import (
        get_agi_accretor,
...
def _get_cached_response(query: str) -> Optional[str]:
    ...
    with _CACHE_LOCK:
        ...
        if (
            len(query.split()) <= 10
        ):  # Only for shorter queries to avoid performance issues
            ...
            for cached_query, entry in _ultra_cache.items():
                ...
                if query_words and cached_words:
                    ...
                    length_ratio = min(len(query_words), len(cached_words)) / max(
                        len(query_words), len(cached_words)
                    )
                    if (
                        similarity >= 0.6
            ...
            if best_match:
                ...
                logger.info(
                    f"⚡ SEMANTIC CACHE HIT! (similarity: {best_score:.2f}, {best_match['hits']} hits)"
                )
...
def _set_ml_analysis(query: str, analysis: Dict[str, Any]):
    ...
    with _CACHE_LOCK:
        _ml_analysis_cache[query] = {"analysis": analysis, "timestamp": time.time()}
        # Limit cache size
        if len(_ml_analysis_cache) > 50:  # Keep only 50 recent analyses
            oldest_key = min(
                _ml_analysis_cache.keys(),
...
def _score_and_report(query: str, response: str) -> tuple[float, str]:
    ...
    try:
        ...
        if _TEMPLATE_REGISTRY:
            try:
                enhanced_response = _TEMPLATE_REGISTRY.enhance_response_with_template(
                    query, response
                )
            except Exception as e:
        ...
        if _ACCRETOR_AVAILABLE:
            try:
                ...
                features = np.array(
                    [
                        [
                            ...
                            len(re.findall(r"\b\d+\b", enhanced_response))
                            / 10.0,  # Numbers count
                            len(re.findall(r"[.!?]", enhanced_response))
                            / 20.0,  # Sentence count
                        ]
                    ]
                )
                ...
                accretor_quality_score = (
                    float(quality_preds[0]) if len(quality_preds) > 0 else 0.5
                )

                logger.info(
                    f"🎯 AI Accretor quality prediction: {accretor_quality_score:.3f}"
                )
        ...
        if _QUALITY_SCORER_AVAILABLE and _GLOBAL_QUALITY_SCORER:
            ...
            if accretor_quality_score is not None:
                ...
                logger.info(
                    f"📊 Combined quality score: {score:.3f} (traditional: {traditional_score:.3f}, accretor: {accretor_quality_score:.3f})"
                )
            else:
            ...
            heuristic_score = (
                min(1.0, max(0.0, response_len / 400.0)) if response_len > 0 else 0.0
            )
        ...
        try:
            from agi_chatbot.self_improvement.self_improvement_module import (
                SelfImprovementModule,
            )

            sim = SelfImprovementModule()
            sim.record_response_evaluation(
                query=query, response=enhanced_response, quality=score
            )
        except Exception:
        ...
        logger.info(
            f"[QUALITY] Response quality for query '{query[:50]}...': {score:.2f}"
        )
...
def _produce_self_improvement_response(user_input: str) -> str:
    ...
    try:
        ...
        if performance.get("status") != "no_data":
            ...
            response_parts.append(
                "• Just initialized - starting to collect data from interactions"
            )
...
def _set_cached_response(query: str, response: str, ttl: Optional[int] = None):
    ...
    with _CACHE_LOCK:
        ...
        if len(_ultra_cache) > 1000:
            # remove oldest
            oldest_key = min(
                _ultra_cache.keys(), key=lambda k: _ultra_cache[k]["timestamp"]
            )
    ...
    if any(pattern in query_lower for pattern in error_patterns):
        ...
        try:
            ...
            if "error:" in query_lower or "exception:" in query_lower:
                ...
                error_part = (
                    user_input.split("error:")[-1]
                    if "error:" in query_lower
                    else user_input.split("exception:")[-1]
                )
            ...
            if diagnosis["success"]:
                response = "🔧 **Error Diagnosis Complete**\n\n"
                response += f"**Detected Error:** {diagnosis['detected_error']}\n"
                response += f"**Description:** {diagnosis['error_description']}\n"
                response += (
                    f"**Applied Correction:** {diagnosis['correction_applied']}\n\n"
                )
                response += "**Status:** ✅ Error resolved automatically"
                return response
            else:
...
def _handle_simple_deterministic_query(user_input: str) -> Optional[str]:
    ...
    query_lower = user_input.lower().strip()
    print(
        f"DEBUG: _handle_simple_deterministic_query called with: '{query_lower}'"
    )  # Debug print
    ...
    is_performance_question = any(
        re.search(pattern, query_lower) for pattern in performance_patterns
    )
    ...
    if query_lower.startswith("calculate ") or query_lower.startswith("calc "):
        ...
        expression = (
            user_input[9:] if query_lower.startswith("calculate ") else user_input[5:]
        )
        try:
    ...
    has_numbers_and_operators = (
        bool(re.search(r"\d", query_lower)) and has_math_operators
    )
    ...
    has_to_with_units = " to " in query_lower and any(
        unit in query_lower
        for unit in [
            "celsius",
            "fahrenheit",
    ...
    if has_convert or has_to_with_units:
        ...
        if (
            "celsius to fahrenheit" in query_lower
            or "c to f" in query_lower
            or ("celsius" in query_lower and "fahrenheit" in query_lower)
        ):
            # Extract number
            ...
            "fahrenheit to celsius" in query_lower
            or "f to c" in query_lower
            or ("fahrenheit" in query_lower and "celsius" in query_lower)
        ):
            numbers = re.findall(r"\d+", query_lower)
            if numbers:
    ...
    # Random numbers
    if (
        query_lower.startswith("random")
        or query_lower.startswith("roll")
        or "random" in query_lower
    ):
    ...
    if any(re.search(pattern, query_lower) for pattern in greeting_patterns):
        ...
        return "Hello! I'm your ultra-fast AGI assistant. How can I help you today?"
...
_ULTRA_LOW_LATENCY = (
    False  # Disable always-on ultra-low latency, use adaptive mode instead
)
...
try:
    from .ai_enhancements import _quality_assessor, _semantic_cache
...
async def _process_standard_enhanced_answer(
    user_input: str,
    chatbot,
    ...
    if _LEVEL_1_AVAILABLE:
        try:
            ...
            logger.info(
                f"✅ Standard answer complete: latency={latency_ms:.0f}ms, quality={quality_score:.2f}"
            )
        except Exception as e:
...
async def enhanced_answer(
    ...
    """
    print(
        f"DEBUG: enhanced_answer called with user_input: '{user_input}'"
    )  # Debug print
    start_time = time.time()
    ...
    try:
        if (
            os.getenv("ENABLE_UNBREAKABLE_ORACLE_EXPERIMENT", "0") == "1"
            and _FRAMEWORK_AVAILABLE
        ):
            try:
            ...
            if framework is not None:
                ...
                try:
                    is_candidate = False
                    try:
                        is_candidate = getattr(
                            chatbot, "is_oracle_candidate", lambda txt: False
                        )(user_input)
                    except Exception:
                        is_candidate = False
                    ...
                    if not is_candidate:
                        is_candidate = (
                            any(
                                ...
                                for k in ("analyze", "explain", "evaluate", "assess")
                            )
                    ...
                    if is_candidate:
                        ...
                        try:
                            ...
                            logging.getLogger().info(
                                "Routing query to EnhancedAGIFramework"
                            )
                            return result
                        except Exception as e:
                            if isinstance(stats, dict):
    ...
    is_simple_query = (
        ...
        and not any(char in user_input for char in ["?", "!", "...", "etc"])
    )

    if is_simple_query and _PERFORMANCE_MODE:
        logger.info("⚡ SIMPLE QUERY: Using lightweight processing path")
        return await _process_standard_enhanced_answer(
            user_input, chatbot, None, user_id
        )
        ...
        if _LEVEL_1_AVAILABLE and chatbot._use_semantic_cache:
            try:
                ...
                if cached and cached.confidence > 0.85:  # High confidence threshold
                    logger.info(
                        f"⚡ SEMANTIC CACHE HIT: {cached.confidence:.2f} confidence"
                    )
    ...
    if _CAPABILITY_DISCLOSURE_AVAILABLE:
        ...
        if is_ai_limitations_query(user_input):
            logger.info(
                "⚠️ AI LIMITATIONS QUERY: Using AI limitations disclosure response"
            )
        ...
        if is_coding_improvement_query(user_input):
            logger.info(
                "💻 CODING IMPROVEMENT QUERY: Using coding improvement disclosure response"
            )
        ...
        if is_speech_improvement_query(user_input):
            logger.info(
                "🗣️ SPEECH IMPROVEMENT QUERY: Using speech improvement disclosure response"
            )
    ...
    if skip_advanced_reasoning:
        ...
        return await _process_standard_enhanced_answer(
            user_input, chatbot, provider, user_id
        )
    ...
    if _PERFORMANCE_MODE and _ADAPTIVE_LATENCY:
        ...
        simple_greeting = bool(
            re.search(
                r"\b(how are you|how's it going|whats up|what's up|hello|hi|hey)\b",
                lower_user,
            )
        )
        ...
        has_reasoning_keywords = (
            any(word in lower_user for word in reasoning_indicators)
        ...
        needs_full_processing = (
            query_complexity > 0.6  # Complex queries
            or has_reasoning_keywords  # Reasoning required
            or has_multiple_questions  # Multi-part questions
            or has_complex_structure  # Structured content
            or query_words > 30  # Long queries
        )
    ...
    if _HYBRID_ARCHITECTURE_AVAILABLE and _hybrid_architecture:
        ...
        query_complexity = (
            len(user_input.split()) / 100.0
        )  # More stringent complexity check
        lower_user = user_input.lower()
        simple_greeting = bool(
            re.search(
                r"\b(how are you|how's it going|whats up|what's up|hello|hi|hey)\b",
        ...
        if query_complexity > 0.8 or (
            has_reasoning_keywords and query_complexity > 0.3
        ):
            ...
            try:
                ...
                if isinstance(hybrid_result, dict):
                    response = hybrid_result.get(
                        "response",
                        "I processed your complex query but encountered an issue.",
                    )
                else:
                ...
                logger.warning(
                    f"Hybrid architecture failed: {e}, falling back to enhanced reasoning"
                )
            ...
            try:
                if (
                    any(tok in user_input.lower() for tok in ethics_tokens)
                    and _ETHICS_REASONING_AVAILABLE
                ):
                    base_draft = generate_ethics_analysis(
                        user_input,
                        {
                            "user_id": user_id,
                ...
                try:
                    ...
                    if not response or len(response) < 10:
                        response = (
                            base_draft
                            or seed
                            or await chatbot.answer(user_input, provider=provider)
                        )
                except Exception as e:
                    logger.warning(f"Advanced reasoning failed: {e}")
                    response = base_draft or await chatbot.answer(
...
def get_enhancement_status() -> dict:
    ...
    return {
        "level_1_core": {
            ...
            "components": (
                ["SemanticCache", "RouterOptimizer", "QualityAssessor"]
                if _LEVEL_1_AVAILABLE
                else []
            ),
        },
        ...
        "advanced_optimization": {
            ...
            "components": (
                ["AdvancedModelPruning", "KnowledgeDistillation", "OptimizationManager"]
                if _ADVANCED_OPTIMIZATION_AVAILABLE
                else []
            ),
        },
        ...
        "total_layers": sum(
            [
                ...
                _LEVEL_3_AVAILABLE,
                _NLP_ML_AVAILABLE,
                _ADVANCED_OPTIMIZATION_AVAILABLE,
                _EREBUS_AVAILABLE,
            ]
        ),
    }
...
def analyze_question_for_learning(question: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if any(
            ...
            for word in ["joke", "funny", "story", "laugh", "entertain"]
        ):
        ...
        # Check generative patterns
            ...
            word in question_lower
            for word in ["generate", "create", "code", "write", "build"]
        ):
            intent = "generative"
        ...
        return {"error": str(e), "question": question, "intent": "unknown"}
