...
async def test_oracle_routing_async(monkeypatch):
    ...
    monkeypatch.setattr(
        enhanced_answer, "get_enhanced_framework", lambda: FakeFrameworkAsync()
    )

    res = await enhanced_answer.enhanced_answer(
        DummyChatbot(),
        "Please analyze this complex scenario and explain",
        None,
        "tester",
    )
...
async def test_oracle_routing_sync(monkeypatch):
    ...
    monkeypatch.setattr(enhanced_answer, "_FRAMEWORK_AVAILABLE", True)
    monkeypatch.setattr(
        enhanced_answer, "get_enhanced_framework", lambda: FakeFrameworkSync()
    )
...
async def test_oracle_routing_fallback_on_exception(monkeypatch):
    ...
    monkeypatch.setattr(enhanced_answer, "_FRAMEWORK_AVAILABLE", True)
    monkeypatch.setattr(
        enhanced_answer, "get_enhanced_framework", lambda: FakeFrameworkRaise()
    )
...
async def test_oracle_routing_metrics_and_logging(monkeypatch, caplog):
    ...
    res = await enhanced_answer.enhanced_answer(
        bot, "Please analyze this complex scenario and explain", None, "tester"
    )
